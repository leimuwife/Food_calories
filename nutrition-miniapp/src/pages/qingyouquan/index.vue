  ...
  <view class="page-container">
    ...
    >
      ...
      <view v-for="feed in feedList" :key="feed.id" class="feed-card">
        ...
        <view v-if="feed.imageUrls && feed.imageUrls.length > 0" class="feed-images">
          <view 
            v-for="(img, idx) in feed.imageUrls" 
            :key="idx" 
            :class="getImageClass(feed.imageUrls.length, idx)"
            @tap="previewImage(feed.imageUrls, idx)"
          >
...
async function toggleLike(feed: FeedItem) {
  try {
    ...
    feed.likeCount = Number(res.data.likeCount)
  } catch (e) {
...
async function submitComment() {
  ...
  try {
    const res = await addFeedComment(currentFeed.value.id, commentInput.value.trim())
    ...
    currentFeed.value.commentCount = Number(res.data.commentCount)
    commentInput.value = ''
